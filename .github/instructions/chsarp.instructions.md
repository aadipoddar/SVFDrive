...
- **Multi-Project Solution**: SVFDrive has multiple projects (MAUI, Web, Shared, Library, API)
- **Shared Library**: Business logic is in `SVFDriveLibrary` - always check here before adding new data access code
- **Database**: SQL Server database project in `DBSVFDrive` with stored procedures and views
- **Excel Import**: Utility project `ExcelImport` for data import operations
- **Code Reuse**: Maximize code sharing between Web and MAUI projects through `SVFDrive.Shared`
...
- **Data Models**: Keep models in `SVFDriveLibrary/Models` for consistency
- **Export Logic**: Implement export functionality in `SVFDriveLibrary/Exports`
- **Styling Consistency**: Reuse global styles from `SVFDrive.Shared/wwwroot/app.css` when available instead of duplicating local page styles
