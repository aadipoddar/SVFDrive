...
- Before adding new local styles, first check for existing reusable styles in `SVFDrive.Shared/wwwroot/app.css`
...
- **Shared Components**: Reusable components are in `SVFDrive.Shared` project
- **Web Application**: Blazor Server/Web app is in `SVFDrive.Web` project  
- **MAUI Hybrid**: Cross-platform mobile/desktop app is in `SVFDrive` project
- **Business Logic**: Shared business logic and data access is in `SVFDriveLibrary` project
- **Component Reusability**: Always check `SVFDrive.Shared/Components` before creating new UI components
- **Service Layer**: Data services are centralized in `SVFDriveLibrary/Data` and `SVFDriveLibrary/DataAccess`
