...
public class PhysicalFileProvider
{
	...
	public void SetRules(AccessDetails details)
	{
		...
		DirectoryInfo root = new(contentRootPath);
		rootName = root.Name;
	}
	...
	public virtual FileManagerResponse GetFiles(string path, bool showHiddenItems, params FileManagerDirectoryContent[] data)
	{
		...
		try
		{
			...
			DirectoryInfo directory = new(safePath);
	...
	protected virtual IEnumerable<FileManagerDirectoryContent> ReadFiles(DirectoryInfo directory, string[] extensions, bool showHiddenItems, params FileManagerDirectoryContent[] data)
	{
		try
		{
			FileManagerResponse readFiles = new();
			if (!showHiddenItems)
			{
	...
	{
		FileManagerResponse readDirectory = new();
		try
		{
			if (!showHiddenItems)
			{
	...
	public virtual FileManagerResponse Create(string path, string name, params FileManagerDirectoryContent[] data)
	{
		...
		try
		{
			...
			if (directoryExist)
			{
				DirectoryInfo exist = new(newDirectoryPath);
				ErrorDetails er = new()
				{
					Code = "400",
					Message = "A file or folder with the name " + exist.Name.ToString() + " already exists."
				};
			...
			DirectoryInfo directory = new(newDirectoryPath);
			FileManagerDirectoryContent CreateData = new()
			{
				Name = directory.Name,
				IsFile = false,
				Size = 0,
	...
	{
		FileManagerResponse getDetailResponse = new();
		FileDetails detailFiles = new();
		try
		{
			string safePath = SanitizePath(Path.Combine(contentRootPath + path));
			if (Path.GetFullPath(safePath) != GetFilePath(safePath))
			...
			if (names.Length == 0 || names.Length == 1)
			{
				...
				DirectoryInfo baseDirectory = new(string.IsNullOrEmpty(hostPath) ? contentRootPath : hostPath);
	...
	{
		FileManagerResponse DeleteResponse = new();
		List<FileManagerDirectoryContent> removedFiles = [];
		try
		{
			string safePath = SanitizePath(Path.Combine(contentRootPath + path));
			if (Path.GetFullPath(safePath) != GetFilePath(safePath))
	...
	public virtual FileManagerResponse Rename(string path, string name, string newName, bool replace = false, bool showFileExtension = true, params FileManagerDirectoryContent[] data)
	{
		...
		try
		{
			...
			if (isFile)
			{
				...
				{
					FileInfo exist = new(newPath);
					ErrorDetails er = new()
					{
						Code = "400",
						Message = "Cannot rename " + exist.Name.ToString() + " to " + newName + ": destination already exists."
					};
			...
			else
			{
				...
				{
					DirectoryInfo exist = new(newPath);
					ErrorDetails er = new()
					{
						Code = "400",
						Message = "Cannot rename " + exist.Name.ToString() + " to " + newName + ": destination already exists."
	...
	public virtual FileManagerResponse Copy(string path, string targetPath, string[] names, string[] renameFiles, FileManagerDirectoryContent targetData, params FileManagerDirectoryContent[] data)
	{
		...
		try
		{
			...
			List<FileManagerDirectoryContent> copiedFiles = [];
			...
			if (result != String.Empty)
			{
				...
				ErrorDetails er = new()
				{
					Message = "'" + getFileNameFromPath(deniedPath) + "' is not accessible. You need permission to perform the copy action.",
					Code = "401"
				};
	...
	public virtual FileManagerResponse Move(string path, string targetPath, string[] names, string[] renameFiles, FileManagerDirectoryContent targetData, params FileManagerDirectoryContent[] data)
	{
		...
		try
		{
			...
			List<string> existFiles = [];
			List<string> missingFiles = [];
			List<FileManagerDirectoryContent> movedFiles = [];
			string tempPath = path;
			for (int i = 0; i < names.Length; i++)
			{
				string fullName = names[i];
			...
			if (result != String.Empty)
			{
				...
				ErrorDetails er = new()
				{
					Message = "'" + getFileNameFromPath(deniedPath) + "' is not accessible. You need permission to perform this action.",
					Code = "401"
				};
	...
	{
		FileManagerResponse searchResponse = new();
		try
		{
			path ??= string.Empty; ;
			...
			string rootPath = string.IsNullOrEmpty(hostPath) ? contentRootPath : new DirectoryInfo(SanitizePath(hostPath)).FullName;
			string parentPath = GetParentPath(path, directory);
			cwd.HasChild = CheckChild(directory.FullName);
			cwd.Type = directory.Extension;
			cwd.FilterPath = GetRelativePath(rootPath, parentPath + Path.DirectorySeparatorChar);
			...
			List<FileManagerDirectoryContent> foundedFiles = [];
			string[] extensions = allowedExtension;
			DirectoryInfo searchDirectory = new(safePath);
			List<FileInfo> files = [];
			...
			if (showHiddenItems)
			{
				...
				{
					FileManagerDirectoryContent dirDetails = GetFileDetails(SanitizePath(Path.Combine(contentRootPath, dir.FullName)));
...
#else
								zipEntry = archive.CreateEntryFromFile(SanitizePath(Path.Combine(contentRootPath, currentDirectory)), names[i], CompressionLevel.Fastest);
#endif
							}
						}
						catch (Exception)
	...
	protected FileStreamResult DownloadFolder(string path, string[] names, int count)
	{
		try
		{
			...
			if (names.Length == 1)
			{
				...
				DirectoryInfo directoryName = new(fullPath);
	...
	protected virtual FileManagerDirectoryContent GetFileDetails(string path)
	{
		try
		{
			...
			FileInfo detailPath = new(info.FullName);
			int folderLength = 0;
	...
	{
		AccessPermission FilePermission = new();
		if (isFile)
		{
	...
	private string GetParentPath(string path, DirectoryInfo currentDirectory)
	{
		...
		{
			string combined = hostPath + (path != "/" ? path : "");
