...
public partial class UserPage
{
	...
	[
		...
		new() { Text = "Reset", Id = "ResetSelectedItem", IconCss = "e-icons e-reset", Target = ".e-content" }
	];
	...
	private async Task SaveTransaction()
	{
		...
		try
		{
			...
			await UserData.SaveTransaction(_transactionUser);
	...
	private async Task ResetSelectedItem()
	{
		...
		var user = await CommonData.LoadTableDataById<UserModel>(OperationNames.User, selectedRecords[0].Id);
		if (user is null)
			await _toastNotification.ShowAsync("Error while Editing", "Transaction Not Found.", ToastType.Error);

		await UserData.ResetInsertUser(user);

		await _toastNotification.ShowAsync("Reset", "Transaction has been reset successfully.", ToastType.Success);
		StateHasChanged();
	}
